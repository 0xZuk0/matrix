...
3. [FootHold](#foothold)
4. [Privilege Escilation](#privilege-escialation)
...
We see that the user can run `admin_tasks.sh` as root. Let's see what's inside the script. After analysing the script the `backup_web()` was looking intresting. 

![image](https://raw.githubusercontent.com/0xZuk0/matrix/master/assets/Admirer/backup_web_code.png)

It was running `backup.py` file which was present in the same directory. Let's look at `backup.py`.
